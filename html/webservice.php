...
switch( $_GET['action'] ) {
    ...
    case 'start_game':
        if( array_key_exists("ships", $_GET) && Battleships::ships_check($_GET['ships']) && $oBattleships->startGame($_GET['ships']) ) {
            $return['success'] = true;
        }
        else {
            $return['error'] = true;
    ...
    # chat
    case 'chat':
        if( array_key_exists("text", $_GET) && $oBattleships->chat( $_GET['text'] ) ) {
    ...
    # when player shoots
    case 'shot':
        $shot = array_key_exists("coords", $_GET) ? $oBattleships->shot( $_GET['coords'] ) : false;
